...
void refine(vector< vector< pair< long int, long int > > > &in_graph, vector< long int > &flags_in, int flag){
	...
	if(flag == 1)
	{
		...
		while(tolerance >= tol)
		{
			...
			if(greater_index == 0){
				max_key = part0.rbegin()->second;
				// printf("max_key 1 = %ld", max_key);
			}
			else{
				max_key = part1.rbegin()->second;
				// printf("max_key 2 = %ld", max_key);
			}
			tolerance = (float)diff/total;
			// long int checknum0 = 0;
			// long int checknum1 = 0;
			// for (long int i = 1; i < flags_in.size(); ++i)
			// {
			// 	if(flags_in[i] == 1)
			// 		checknum1++;
			// 	else
			// 		checknum0++;
			// }
			// cout << "num0 = " << num0 << "\t num1 = " << num1 << "     ";
			// cout << "num0 = " << checknum0 << "\t num1 = " << checknum1 << endl;
		}
	}
}
